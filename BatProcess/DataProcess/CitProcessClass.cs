...
namespace InGraph.DataProcessClass
{
    public class CitProcessClass
    {
        ...
        private Byte[] CreateChannelInfo(String citFilePath, List<Byte> sectionChannelId, string exportPath, out string sectionFilePath)
        {
            ...
            sectionFileName.AppendFormat(".{0}", tmpStrCitFileName[1]);
            //sectionFilePath = Path.Combine(citFileDiretory, sectionFileName.ToString());
            sectionFilePath = exportPath + sectionFileName.ToString();
            ...
            try
            {
                using (FileStream fs = new FileStream(citFileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                {
                    using (BinaryReader br = new BinaryReader(fs, Encoding.Default))
                    {
                        ...
                        if (dhi.iDataType != 2)
                        {
                            throw new Exception("该波形文件不是动力学波形文件！");
        ...
        private void ProcMaxIn50M(String sectionFilePath,int segmentLen,string idfOutPath,out string idfPath)
        {
            /*
            ...
            String idfFilePath = sectionFilePath.Substring(0, sectionFilePath.LastIndexOf("\\") + 1) + idfFileName;

            idfFilePath = idfOutPath + idfFileName;
        ...
        /// <param name="sectionFilePath">加速度断面cit文件的全路径文件名</param>
        /// <param name="segmentLen">区段的段长</param>
        private void ProcMaxIn50M_Fr_CB(String sectionFilePath, int segmentLen, string idfOutPath)
        {
            /*
        ...
        private void ProcMaxIn50M_NoCreateIdf(String sectionFilePath, int segmentLen, string idfOutPath, out string idfPath)
        {
            /*
             * 从断面cit文件中计算出每个有效值通道的50米区段大值
             * 把50米区段大值保存在idf文件中
            ...
            try
            {
                using (FileStream fs = new FileStream(sectionFilePath, FileMode.Open, FileAccess.Read, FileShare.Read))
                {
                    using (BinaryReader br = new BinaryReader(fs, Encoding.Default))
                    {
                        ...
                        string txtFilesPath = idfOutPath + sectionFileName + "_Rms";
                        if (!Directory.Exists(txtFilesPath))
                        {
                            Directory.CreateDirectory(txtFilesPath);
                        }
                        WriteToTxtCitFileInfo(txtFilesPath + "\\CitFileInfo.txt" , m_dhi);
                        ...
                        for (int n = 0; n < rmsChannelIdList.Count; n++)
                        {
                            ...
                            //InsertIntoSegmentRmsTable(idfFilePath, segmentRmsTableNames[n], list_AB_KM, list_AB_SPEED, list_AB_RMS[n], list_AB_RMS_PEAK[n]);
                            txtFilesPath = idfOutPath + sectionFileName + "_Rms";
                            if (!Directory.Exists(txtFilesPath))
                            {
                                Directory.CreateDirectory(txtFilesPath);
                            }

                            SaveIntoSegmentRmsTxtHeader(txtFilesPath + "\\" + segmentRmsTableNames[n] + ".txt");

                            SaveIntoSegmentRmsTxt(txtFilesPath + "\\" + segmentRmsTableNames[n] + ".txt", list_AB_KM, list_AB_SPEED, list_AB_RMS[n], list_AB_RMS_PEAK[n]);
                            resultArrayAB_RMS_PEAK_REAL = null;
                        }
        ...
        private void ProcMaxIn50M_Fr_CB_NoCreateIdf(String sectionFilePath, int segmentLen, string idfOutPath)
        {
            /*
             * 架构和车体的50米区段大值的原理和轴箱不一样
             * 架构和车体取的是50米范围内绝对值的最大值的所在位置的点的原值（有正负号）
        ...
        private void CreateDB(string idfFilePath)
        {
            try
            {
                DataAccess.AccessHelper.CreateAccessDb(idfFilePath);
            }
            catch (Exception ex)
            {
                throw new Exception(ex.ToString());
        ...
        private void CreateTable(string idfFilePath,String tableName)
        {
            ...
            //{
            //    ADOX.Column[] columns = {
            //                                 new ADOX.Column(){Name="KiloMeter",Type=ADOX.DataTypeEnum.adLongVarWChar,DefinedSize=0},
            //                                 new ADOX.Column(){Name="Speed",Type=ADOX.DataTypeEnum.adLongVarWChar,DefinedSize=0},
            //                                 new ADOX.Column(){Name="Segment_RMS",Type=ADOX.DataTypeEnum.adLongVarWChar,DefinedSize=0},
            //                                 new ADOX.Column(){Name="Segment_RMS_Peak",Type=ADOX.DataTypeEnum.adLongVarWChar,DefinedSize=0},
            //                                 new ADOX.Column(){Name="valid",Type=ADOX.DataTypeEnum.adInteger,DefinedSize=0},
            //                             };

            //    DataAccess.AccessHelper.CreateAccessTable(idfFilePath, tableName, columns, "Id");
            //}
            //catch (Exception ex)
            //{
            //    throw new Exception(ex.ToString());
        ...
        private void CreateTableCitFileInfo(string idfFilePath)
        {
            ...
            //{
            //    ADOX.Column[] columns = {
            //                                 new ADOX.Column(){Name="LineName",Type=ADOX.DataTypeEnum.adLongVarWChar,DefinedSize=0},
            //                                 new ADOX.Column(){Name="LineCode",Type=ADOX.DataTypeEnum.adLongVarWChar,DefinedSize=0},
            //                                 new ADOX.Column(){Name="LineDir",Type=ADOX.DataTypeEnum.adLongVarWChar,DefinedSize=0},
            //                                 new ADOX.Column(){Name="KmInc",Type=ADOX.DataTypeEnum.adLongVarWChar,DefinedSize=0},
            //                                 new ADOX.Column(){Name="SDate",Type=ADOX.DataTypeEnum.adDate,DefinedSize=0},
            //                                 new ADOX.Column(){Name="STime",Type=ADOX.DataTypeEnum.adDate,DefinedSize=0},
            //                                 new ADOX.Column(){Name="Train",Type=ADOX.DataTypeEnum.adLongVarWChar,DefinedSize=0}
            //                             };
        ...
        private void WriteToTxtCitFileInfo(string txtFilePath, DataHeadInfo m_dhi)
        {
            InitDicKmInc();//初始化字典
            InitDicDir();
            ...
            using (StreamWriter sw = File.AppendText(txtFilePath))
            {
                ...
                header = "Id,LineName,LineCode,LineDir,KmInc,SDate,STime,Train";
                sw.WriteLine(header);
                content = String.Format("{0},{1},{2},{3},{4},{5},{6}",
                    1, m_dhi.sTrackName, dhi.sTrackCode, dicDir[m_dhi.iDir], dicKmInc[m_dhi.iKmInc], m_dhi.sDate, m_dhi.sTrain);
        ...
        public void UpdatePeak(String rmsIdfPath, String tableName, double[] peakArray)
        {
            ...
            {
                throw new Exception("写入峰值因子出错:" + ex.ToString());
            }
        }


        private void SaveIntoSegmentRmsTxtHeader(string txtFilePath)
        {
            using (StreamWriter sw = File.AppendText(txtFilePath))
            {
                string header = "Id,KiloMeter,Speed,Segment_RMS,Segment_RMS_Peak,Valid";
                sw.WriteLine(header);
            }
        }

        private void SaveIntoSegmentRmsTxt(string txtFilePath, List<double> data_KM, List<double> data_SPEED, List<double> data_RMS, List<double> data_RMS_Peak)
        {
            using (StreamWriter sw = File.AppendText(txtFilePath))
            {
                string content = "";
                ...
                {
                    content = (i + 1).ToString() + "," + data_KM[i].ToString() + "," + data_SPEED[i].ToString() + "," + data_RMS[i].ToString() + "," + data_RMS_Peak[i].ToString() + ",1";
                    sw.WriteLine(content);
                }
            }
        }
        ...
        public string CalcSegmentRMS(List<String> sctnFilePathList,int segmentLen,string idfOutPath)
        {
            string idfFilePath = "";
            foreach (String sctnFilePath in sctnFilePathList)
            {
                ProcMaxIn50M(sctnFilePath, segmentLen,idfOutPath, out idfFilePath);
                ProcMaxIn50M_Fr_CB(sctnFilePath, segmentLen, idfOutPath);
            }

            return idfFilePath;
        }

        public string CalcSegmentRMS_NoCreateIdf(List<String> sctnFilePathList, int segmentLen, string idfOutPath)
        {
            string idfFilePath = "";
            foreach (String sctnFilePath in sctnFilePathList)
            {
                ProcMaxIn50M_NoCreateIdf(sctnFilePath, segmentLen, idfOutPath, out idfFilePath);
                ProcMaxIn50M_Fr_CB_NoCreateIdf(sctnFilePath, segmentLen, idfOutPath);
            }
        ...
        public List<String> CreateAllSectionFileNew(String citFilePath, int sampleFreq, int maxFreq, int minFreq, int windowLen, List<double> upperChannelFreq, List<double> lowerChannelFreq,string exportPath)
        {
            List<String> m_sectionFilePathList = new List<String>();
            ...
            {
                String SectionFilePath = null;
                Byte[] SectionChannelInfo = CreateChannelInfo(citFilePath, channelIdList,exportPath, out SectionFilePath);
                mCitHeadInfo = CreatHeadInfo(citHeadInfoList, (int)(SectionChannelInfo.Length / 65));
                //CreateSectionFile(citFilePath, channelIdList, mCitHeadInfo.ToArray(), SectionChannelInfo, SectionFilePath, sampleFreq, maxFreq, windowLen);
                CreateSectionFileNew(citFilePath, channelIdList, mCitHeadInfo.ToArray(), SectionChannelInfo, SectionFilePath, sampleFreq, maxFreq, minFreq, windowLen, upperChannelFreq, lowerChannelFreq);
                SectionChannelInfo = null;
